...
class Notion implements INotion {
  ...
  async queryPages(options: QueryPagesOptions = {}): Promise<NotionPage[]> {
    ...
    try {
      ...
      throw new NotionException(`Failed to query Notion pages`, error);
    }
  }
  ...
  async getPageHtmlAndImage(pageId: string): Promise<GetPageHtmlAndImageResponse> {
    ...
    try {
      let mdBlocks: MdBlock[] = await retryWithBackoff(() => this.n2m.pageToMarkdown(pageId), {
        onRetry: onRetryFn,
      });
  ...
  async updatePageStatus(
    ...
    try {
      ...
      logger.debug(`notion - Updated page ${pageId} status to: ${status}`);
      ...
      throw new NotionException(`Failed to update page status`, error);
    }
  }
...
export const notion: INotion = new Notion();
