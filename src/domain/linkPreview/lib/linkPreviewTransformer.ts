...
import { linkPreviewMetadataFetcher } from '../impl/linkPreviewMetadataFetcherImpl.js';
...
function wrapWithWordPressHtmlBlock(html: string): string {
  return `<!-- wp:html -->\n${html}\n<!-- /wp:html -->`;
}

function renderFallbackBookmark(url: string): string {
  try {
    return wrapWithWordPressHtmlBlock(renderBookmarkHTML(fallbackMetadata(url)));
  } catch {
    const escapedHref = escapeHtml(safeHref(url));
    const escapedText = escapeHtml(url);

    return `<figure class="bookmark-card"><a href="${escapedHref}" target="_blank" rel="noopener noreferrer">${escapedText}</a></figure>`;
  }
}
...
async function renderBookmarkCard(url: string): TransformerResult {
  try {
    ...
    return wrapWithWordPressHtmlBlock(renderBookmarkHTML(metadata));
  } catch {
