...
describe('TelegramService', () => {
  ...
  it('disabled -> skips sending', async () => {
    ...
    await telegramService.sendSyncNotification('hello');
  ...
  it('formats SyncJob message', async () => {
    ...
    const telegramService = await loadTelegramService();
    await telegramService.sendSyncNotification(job as any);
  ...
  it('formats string message', async () => {
    ...
    const telegramService = await loadTelegramService();
    await telegramService.sendSyncNotification('something happened');
    const args = sendMessageMock.mock.calls[0]!;
  ...
  it('send failure does not throw', async () => {
    ...
    await expect(telegramService.sendSyncNotification('x')).resolves.toBeUndefined();
  });
});
